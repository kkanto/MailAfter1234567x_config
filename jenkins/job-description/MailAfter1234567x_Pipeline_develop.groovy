...
def job_config = [
        job: [
                name: "MailAfter1234567x_Pipeline_develop"
        ],
        git: [
                branch: "develop"
        ]
...
def customLib = library identifier: 'MailAfter1234567x_JSL@develop', retriever: modernSCM(
  [\$class: 'GitSCMSource',
   remote: 'https://github.developer.allianz.io/kkanto/MailAfter1234567x_lib.git',
   credentialsId: 'git-token-credentials']) 
...
def jslCustom     = lib.de.allianz.MailAfter1234567x.new()
    ...
    stages {
        ...
        stage('Checkout') {
            steps {
                echo "checkout"
                script {
                    jslGit.checkout( config, "JEQP", "MailAfter1234567x_Pipeline_develop", "develop")
                }
            }    
        }
        stage('Build') {
